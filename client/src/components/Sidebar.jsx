...
import {Eraser, FileText, HashIcon, House, Image, LogOut, Scissors, SquarePen, Users } from 'lucide-react'
  ...
  return (
    <div className={`w-60 bg-white border-r border-gray-200 flex flex-col justify-between items-center max-sm:absolute top-14 bottom-0 ${sidebar ? 'translate-x-0' : 'max-sm:-translate-x-full'} transition-all duration-300 ease-in-out`}>
        ...
        <div className='w-full border-t border-gray-300 p-4 px-7 flex items-center justify-between'>
            ...
            <LogOut onClick={signOut} className='2-4.5 text-gray-400 hover:text-gray-700 transition cursor-pointer' />    
        </div>
    </div>
  )
}
